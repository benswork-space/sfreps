...
export default function HomeContent() {
    ...
    setTimeout(() => {
      ...
      if (!map.getSource("districts")) {
        ...
        map.addLayer({
          ...
          paint: {
            "fill-color": ["case", ["==", ["get", "district"], result.district], "rgba(59,130,246,0.35)", "rgba(0,0,0,0.05)"],
            "fill-opacity": 0.7,
          },
        });
        map.addLayer({
          ...
          paint: {
            "line-color": ["case", ["==", ["get", "district"], result.district], "#2563eb", "#94a3b8"],
            "line-width": ["case", ["==", ["get", "district"], result.district], 3, 1],
          },
        });
      }
      ...
      map.once("moveend", () => {
        ...
        setTimeout(() => router.push(`/zip/${result.zip}/${result.supervisorId}`), 2200);
      });
    }, 600);
  }, [router]);
  ...
  return (
    <>
      ...
      {/* Overlay + Content — single layer, no z-index stacking issues */}
      {!overlayFading && (
        <div
          style={{
            position: "fixed",
      ...
      {(phase === "overlay" || phase === "fade") && transition && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl px-8 py-6 text-center shadow-xl flex flex-col items-center">
            {transition.photoUrl && (
              ...
              <img src={transition.photoUrl} alt={transition.supervisorName} className="w-20 h-20 rounded-full object-cover mb-3 ring-2 ring-white shadow-md" />
            )}
            <p className="text-sm font-medium text-zinc-500 uppercase tracking-wider">District {transition.district}</p>
            <p className="text-2xl font-bold text-zinc-900 mt-1">{transition.supervisorName}</p>
      ...
      {/* Fade to white */}
      {phase === "fade" && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 60, background: "white" }} className="animate-fade-in" />
      )}
    </>
  );
}
